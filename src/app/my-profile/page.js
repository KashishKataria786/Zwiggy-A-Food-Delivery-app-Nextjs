    ...
    <CustomerLayout>
      <div className="w-[60vw] mx-auto px-1 py-9">
        ...
        </div>
        {
            ...
            {!loading&&<h1 className='text-xl font-semibold'>{`${menu.length}  items in Menu`}</h1> } 
             <div>
             {loading? <>
             {[1,2,3,4,5].map((index)=><div key={index} className='h-[200px] w-full my-2 bg-gray-200 animate-pulse'>
                </div>)}
